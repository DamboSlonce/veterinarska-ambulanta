...
namespace SlojPodataka
{
    public static class Konekcija
    {
        ...
        public static string String
        {
            get
            {
                ...
                foreach (var server in serveri)
                {
                    try
                    {
                        KreirajBazuAkoNePostoji(server);
                        _cached = $@"Data Source={server};Initial Catalog=Veterinarska_Ambulanta;Integrated Security=True";
                        return _cached;
                    }
                    catch
                    {
                ...
                throw new SqlExceptionPorukaNijeDostupna();
            }
        }
        ...
        private static void KreirajBazuAkoNePostoji(string server)
        {
            var master = $@"Data Source={server};Initial Catalog=master;Integrated Security=True;Connect Timeout=4";
            using (var conn = new SqlConnection(master))
            {
                conn.Open();
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = "IF DB_ID(N'Veterinarska_Ambulanta') IS NULL CREATE DATABASE Veterinarska_Ambulanta;";
                    cmd.ExecuteNonQuery();
                }
            }

            var baza = $@"Data Source={server};Initial Catalog=Veterinarska_Ambulanta;Integrated Security=True;Connect Timeout=8";
            using (var conn = new SqlConnection(baza))
            {
                conn.Open();
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = SkriptaTabela;
                    cmd.ExecuteNonQuery();
                }
            }
        }

        private const string SkriptaTabela = @"
IF OBJECT_ID(N'dbo.Veterinar', N'U') IS NULL
BEGIN
...
IF OBJECT_ID(N'dbo.Pacijent', N'U') IS NULL
BEGIN
...
IF OBJECT_ID(N'dbo.Termin', N'U') IS NULL
BEGIN
    CREATE TABLE Termin (
...
IF OBJECT_ID(N'dbo.Lek', N'U') IS NULL
BEGIN
    CREATE TABLE Lek (
...
";
    ...
    internal class SqlExceptionPorukaNijeDostupna : System.Exception
    {
        public SqlExceptionPorukaNijeDostupna()
            : base("Nije pronađen SQL Server. Instalirajte SQL Server Express (localhost\\SQLEXPRESS) ili LocalDB koji dolazi uz Visual Studio.")
        {
        }
    }
}
